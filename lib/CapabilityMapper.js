...
const { FDM_STATUS_MAP } = require('./SDCPCommands');
...
class CapabilityMapper {
  ...
  static updateStatus(device, attr) {
    if (attr.CurrentStatus === undefined) return null;

    const rawStatus = Array.isArray(attr.CurrentStatus) ? attr.CurrentStatus[0] : attr.CurrentStatus;
    const status = FDM_STATUS_MAP[rawStatus] ?? `Unknown (${rawStatus})`;
    // device.log(`Mapped status code ${rawStatus} to: ${status}`);
